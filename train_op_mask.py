...
def train_one_epoch(img_input,model,optimizer,writer,epoch,args,loss_function,lr_scheduler):
    ...
    '''start training'''
    for each_batch, (img, target_heatmap, heat_mask, target_paf, paf_mask, paf_self_mask) in enumerate(img_input):
        ...
        paf_self_mask = paf_self_mask.cuda()
        paf_mask_final = torch.cat([paf_mask,paf_self_mask],1)
        paf_mask_final = paf_mask_final.cuda()
