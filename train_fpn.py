...
from .datasets import mainloader2
from .network.openpose import resnet_op,resnet_loss
...
def cli():
    ...
    resnet_op.network_cli(parser)
    resnet_loss.loss_cli(parser)
    mainloader2.loader_cli(parser)
    ...
    parser.add_argument('--per_batch',      default=1,          type=int,       help='batch size per gpu')
...
def main():
    ...
    '''data portion'''
    train_loader = mainloader2.train_factory('train',args)
    val_loader = mainloader2.train_factory('val',args)
...
def load_checkpoints(model,optimizer,lr_scheduler,args):
    ...
    try:
        ...
        model_state = model_zoo.load_url(args.weight_res50, model_dir=args.weight_pre)
        res_keys = model_state.keys()
        pretrain_state = {}
        for i in range(len(model_state)):
            pretrain_state[list(model.state_dict().keys())[i]
                    ] = model_state[list(res_keys)[i]]
